...
class ContrastiveLoss(nn.Cell):
    ...
    def forward(self, inputs, targets):
        ...
        sim_mat = ms.ops.matmul(inputs, inputs.t())
        targets = targets
        loss = list()
        c = 0

        for i in range(n):
            pos_pair_ = ms.ops.masked_select(sim_mat[i], targets == targets[i])

            #  move itself
            pos_pair_ = ms.ops.masked_select(pos_pair_, pos_pair_ < 1)
            neg_pair_ = ms.ops.masked_select(sim_mat[i], targets != targets[i])

            pos_pair_ = ms.ops.sort(pos_pair_)[0]
            neg_pair_ = ms.ops.sort(neg_pair_)[0]

            neg_pair = ms.ops.masked_select(neg_pair_, neg_pair_ > self.margin)

            neg_loss = 0

            pos_loss = ms.ops.sum(-pos_pair_ + 1)
            if len(neg_pair) > 0:
                neg_loss = ms.ops.sum(neg_pair)
...
class CircleLoss(nn.Cell):
    def __init__(self, in_features, num_classes, s=256, m=0.25):
        ...
        self.weight = ms.Parameter(ms.Tensor(num_classes, in_features))
        self.s = s
        self.m = m
    ...
    def __call__(self, bn_feat, targets):
        ...
        sim_mat = ms.ops.dense(ms.ops.normal(bn_feat), ms.ops.normal(self.weight))
