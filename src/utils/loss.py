...
class PTCRLoss(nn.Cell):
    ...
    def construct(self, logits, labels):
        ...
        if cfg.MODEL.IF_LABELSMOOTH == 'on':
            if cfg.MODEL.LOSS_PCTR == 'True':
                print('here i am')
                # [score_g,score_l1,l2,,,,] [global_g,global_l1,l2,,,,]
                def loss_global(feature_global, score_global):
                    ID_LOSS = self.ce(score_global, target)
                    TRI_LOSS = self.tri(feature_global, target)
                    # move away [0] TRI_LOSS = self.tri(feature_global, target)[0]
                    return ID_LOSS + TRI_LOSS

                def loss_local(features_local, scores_local):
                    ID_LOSS = [self.ce(score_local, target) for score_local in scores_local]
                    TRI_LOSS = [self.tri(feature_local, target)[0] for feature_local in features_local]
                    return (ms.Tensor(ID_LOSS)[None, :] + ms.Tensor(TRI_LOSS)[:, None]).sum() / (
                            len(ID_LOSS) * len(TRI_LOSS))
                    # ID_LOSS = sum(ID_LOSS) / len(ID_LOSS)
                    # TRI_LOSS = sum(TRI_LOSS) / len(TRI_LOSS)

                return loss_global(feat[0], score[0]) + loss_local(feat[1:], score[1:])
            else:
                if isinstance(score, list):
                    ID_LOSS = [self.ce(scor, target) for scor in score[1:]]
                    ID_LOSS = sum(ID_LOSS) / len(ID_LOSS)
                    ID_LOSS = 0.5 * ID_LOSS + 0.5 * self.ce(score[0], target)
                else:
                    ID_LOSS = self.ce(score, target)
                ...
                if isinstance(feat, list):
                    ...
                    TRI_LOSS = self.tri(feat, target)[0]
