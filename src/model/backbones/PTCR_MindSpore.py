...
from mindspore.common.initializer import initializer, Constant, TruncatedNormal, Normal
...
class Mlp(nn.Cell):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.proj = nn.Conv2d(hidden_features, hidden_features, kernel_size=3, stride=1, padding=1,
                              pad_mode='pad', group=hidden_features)
...
class BlurPool(nn.Cell):
    def __init__(self, channels, pad_type='reflect', filt_size=4, stride=2, pad_off=0):
        ...
        self.filt = ms.Parameter(filt[None, None, :, :].tile((self.channels, 1, 1, 1)).astype(ms.dtype.float32), name='filt',
                                 requires_grad=False)
        # �������Ĭ����float64��������������ֵ��float32����������ͳһ�����Ե�ǿ��ת������Ϊfloat32���ͣ�����
...
class PTCR(nn.Cell):
    ...
    def forward_features(self, x, label=None, view_label=None, cam_label=None):
        B = x.shape[0]
        maps = []
        ...
        for i in range(self.num_stages):
            ...
            # print('before x, H, W = patch_embed(x)')
            # print(x.shape)
            # print('pathc_embed')
            # print(patch_embed)
            x, H, W = patch_embed(x)
            # print('after x, H, W = patch_embed(x)')
            # print(x.shape)
            if i == 0:
                D = x.shape[2]
    ...
    def construct(self, x, label=None, view_label=None, cam_label=None):
        ...
        # print(x.shape)
        feature, maps = self.forward_features(x, label, view_label, cam_label)
